-- UX-001R final: manual payment RPCs already emit the actionable notification.
-- Avoid a second generic notification from the order status trigger.
...
begin
  ...
  if new.status is distinct from old.status then
    ...
    -- Manual payment RPCs send one specific notification to the party that must act.
    if new.payment_method in ('cash', 'wallet', 'transfer')
...
-- Remove only generic manual-payment notifications superseded by actionable copy.
delete from public.notifications n
using public.orders o
where n.order_id = o.id
  and n.kind = 'payment_status'
  and n.title = 'Pago actualizado'
  and n.body = 'El estado de pago del pedido cambió.'
  and o.payment_method in ('cash', 'wallet', 'transfer');
