  ...
  try {
    for(const width of [390,1440]) {
      ...
      for(const actor of ['client','pro']) {
        ...
        await page.getByRole('button',{name:actor==='pro'?'Trabajos':'Pedidos',exact:true}).click();
      ...
      if (width === 390) {
        ...
        try {
          ...
          const o=await rpc(A,'create_scheduled_order',{p_data:{service_id:f.serviceId,description:'NORM014 mobile realtime',address:'NORM014 test',scheduled_at:date.toISOString(),estimated_duration_minutes:60,payment_method:'cash'}});
    ...
    await retryPage.locator('.v6-mode-switch').waitFor({timeout:30000});
