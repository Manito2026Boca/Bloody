...
(async () => {
  ...
  try {
    for (const width of [390, 1440]) {
      ...
      await page.locator('.v6-request-stage textarea').fill('Trabajo de prueba de cobertura sin GPS, no contratar.');
      await page.getByRole('button', { name: 'Continuar', exact: true }).click();
      await page.getByRole('button', { name: 'Usar ubicación del teléfono', exact: true }).click();
      ...
      await page.getByRole('button', { name: 'Continuar', exact: true }).click();
      await page.locator('.v6-mode-row').filter({ hasText: 'Presupuestar' }).click();
      await page.getByRole('button', { name: 'Continuar', exact: true }).click();
      await page.getByRole('button', { name: 'Continuar', exact: true }).click();
      const requestPromise = page.waitForRequest(r => r.method() === 'POST' && r.url().includes('/rest/v1/orders'));
      await page.getByRole('button', { name: 'Publicar solicitud de presupuesto', exact: true }).click();
